...
# saving the model
save_dir = "results/"
model_name = "Sigmo_25_ephocs_weights.h5"
...
# saving the model
save_dir = "results/"
model_name = "Sigmo_40_ephocs_weights.h5"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)
...
# saving the model
save_dir = "results/"
model_name = "model_Relu_32_batch_weights.h5"
model_path = os.path.join(save_dir, model_name)
...
# saving the model
save_dir = "results/"
model_name = "normalization_weights.h5"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)
...
#--------------------------Impelment your code here:-------------------------------------
def get_net_filters(input_shape,drop,dropRate,reg,filters_list):
    #Defining the network architecture:
    model = Sequential()
    model.add(Permute((1,2,3),input_shape = input_shape))
    ...
    for i in range(len(filters_list)):
        model.add(Conv2D(filters=filters_list[i], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_'+str(i+2),kernel_regularizer=regularizers.l2(reg)))
        if drop:
            model.add(Dropout(rate=dropRate))
        model.add(BatchNormalization(axis=1))
        if i%2 != 0:
...
NNet_filters=get_net_filters(input_shape,drop,dropRate,reg,np.int_([64/2,128/2,28/2,256/2,256/2]))
#Compile the network:
NNet_filters.compile(optimizer=AdamOpt, metrics=['acc'], loss='categorical_crossentropy')
h_filters = NNet_filters.fit(x=BaseX_train, y=BaseY_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_split=0, validation_data = (BaseX_val, BaseY_val), shuffle=True)
results_filters = NNet_filters.evaluate(X_test,Y_test)
print('test loss, test acc:', results_filters)
#----------------------------------------------------------------------------------------
